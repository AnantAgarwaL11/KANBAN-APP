...
export default function BoardId({
  ...
  useEffect(() => {
    if (!currentList.length || typeof window === 'undefined') {
      return;
    }
    ...
    const initSocket = async () => {
      try {
        ...
        socket.on('connect', () => {
          console.log('[WS] Connect => Connection established');
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  try {
    ...
    try {
      const data: FetchBoard = await ApiCall(`/api/boards/${id}`);
